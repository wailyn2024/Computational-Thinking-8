##################################
### SETUP ###
import codesters
from codesters import StageClass
stage=StageClass ()
#################################


stage.set_background("winter")


q1=codesters.Square(100, 100, 200, 'blue',)
q2=codesters.Square(-100, 100, 200, 'pink')
q3=codesters.Square(-100, -100, 200, 'purple')
q4=codesters.Square(100, -100, 200, 'navy blue')
s1=codesters.Sprite("cardinal", 100,100,)
s2=codesters.Sprite("dog",100,-100)
s2.set_size(0.2)
s4=codesters.Sprite("headphones", -100, 100)
s4.set_size(0.4)
s5=codesters.Sprite("ford", -100, -100)
s4.set_size(0.5)

message1 = codesters.Text ("My name is Keegan Benisa",0,220, "red")
message2 = codesters. Text ("I go to SAAS",0,-220, "black")